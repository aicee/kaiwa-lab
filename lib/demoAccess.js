...
const MAX_LIVE_SESSION_STARTS = 5;
...
export function reserveLiveVoiceSession(sessionId) {
  ...
  if (usage.voiceSessions >= MAX_LIVE_SESSION_STARTS) {
...
export function checkLiveFeedbackLimits(sessionId, sessionData) {
  ...
  const liveMessages = transcript.filter((message) => message.role === "user" || message.speaker === "user").length;
