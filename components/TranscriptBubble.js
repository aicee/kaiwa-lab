export default function TranscriptBubble({ message, romaji, translation }) {
  return <div className={`transcript-row ${message.speaker}`}>
    ...
    <div><small>{message.speaker === "ai" ? "HANASU AI" : "YOU"}</small><div className="chat-bubble"><p>{message.jp}</p>{romaji && <i>{message.romaji}</i>}{translation && <span>{message.en}</span>}</div></div>
  </div>;
}
