...
export default function FeedbackReport({ scenario, settings, feedback, onRestart, onHarder, onNext }) {
  ...
  const copy = () => { navigator.clipboard?.writeText(`Kaiwa Lab — ${scenario.name}: ${f.score}/100. ${f.wins.join(" ")}`); setCopied(true); setTimeout(()=>setCopied(false),1500); };
  return <div className="app-screen feedback-page"><div className="report-shell">
    ...
    <div className="session-summary"><div><small>SCENARIO</small><b>{scenario.name}</b></div><div><small>LEVEL</small><b>{settings.level}</b></div><div><small>MODE</small><b>{settings.mode}</b></div><div><small>DURATION</small><b>04:18</b></div></div>
