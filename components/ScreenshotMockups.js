import { ArrowRight, CheckCircle2, MessageCircle, Mic } from "lucide-react";
...
export default function ScreenshotMockups({ onDemo }) {
  return <section className="section mockup-section" id="demo"><div className="shell">
    ...
    <div className="mockups">
      <div className="mock-frame small"><div className="browser-dots">● ● ●</div><div className="mock-body"><small>01 — SET YOUR PRACTICE</small><h3>Ramen Shop</h3><div className="mock-option selected">N5 Beginner <CheckCircle2/></div><div className="mock-option">N3 Intermediate</div><div className="mock-option">N1 Advanced</div><span className="mock-label">PRACTICE MODE</span><div className="mock-pills"><b><Mic/> Voice</b><b><MessageCircle/> Text</b></div><button>Start practice →</button></div></div>
      <div className="mock-frame large"><div className="browser-dots">● ● ●</div><div className="mock-body conversation-mock"><div className="mock-sidebar"><b>ラーメン屋</b>{["Find a seat", "Ask recommendation", "Order ramen", "Ask for water"].map((x,i)=><span className={i<2?"done":""} key={x}>{i<2?"✓":"○"} {x}</span>)}</div><div className="mock-chat"><small>RAMEN SHOP · N5</small><div className="tiny-orb"><Mic/></div><div className="tiny-bubble">いらっしゃいませ。<br/>ご注文はお決まりですか？</div><div className="tiny-user">おすすめは何ですか？</div></div></div></div>
