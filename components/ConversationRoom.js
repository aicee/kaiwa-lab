...
export default function ConversationRoom({ scenario, settings, onEnd }) {
  ...
  const transcript = isTextMode ? textTranscript : scenarioTranscript.slice(0, count);
  ...
  return <div className="app-screen conversation-page">
    <header className="room-header"><div className="room-brand"><span>話</span> Kaiwa Lab</div><div className="room-title"><small>NOW PRACTICING</small><b>{scenario.name}</b><span>{scenario.role}</span></div><div className="room-badges"><span>{settings.level.split(" ")[0]}</span><span>{settings.politeness}</span><span>{settings.mode}</span><b>{String(Math.floor(seconds/60)).padStart(2,"0")}:{String(seconds%60).padStart(2,"0")}</b><button onClick={endSession} disabled={ending}>{ending ? <AudioLines/> : <Square/>} {ending ? "Generating feedback…" : "End session"}</button></div></header>
    <div className="room-layout">
      ...
      <section className="conversation-main">
        ...
        {settings.mode === "Demo Mode" && <div className="demo-banner"><Play/> <b>Demo Mode</b> — sample conversation only <span>Step {count} of {scenarioTranscript.length}</span></div>}
        ...
        {settings.mode === "Demo Mode" && count < scenarioTranscript.length && <button className="btn btn-red demo-next" onClick={next}>Play next message <Play/> </button>}
      ...
      <aside className="help-panel">
        ...
        <button onClick={()=>setHelp({type:"hint",label:"TRY SAYING",jp:scenario.usefulPhrases[Math.min(2,scenario.usefulPhrases.length-1)],romaji:"",en:"Optional phrase you can adapt to your answer."})}><Lightbulb/><div><b>Give me a hint</b><span>See a useful phrase</span></div></button>
